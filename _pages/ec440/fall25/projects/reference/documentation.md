---
title: ""
permalink: /EC440/fall25/projects/reference/documentation
...
* <span style="color:blue;">**Function: void thread\_join (tid\_t tid)**</span>
  ...
  * **A thread may only join its immediate children.** **Calling `thread_join()` on a thread that is not the caller's child should cause the caller to return immediately.** <span style="color:red;">**Children are not "inherited"**</span>, that is, if A has child B and B has child C, then A always returns immediately should it try to join C, even if B is dead.
